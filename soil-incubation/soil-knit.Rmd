...
Schädel et al. (2013) performed a soil incubation experiment in which they took a series of 25 measurements over time of evolved CO<sub>2</sub>.  At each time point, multiple readings were taken from replicates and aggregated into a mean and standard deviation.  

The data is loaded here through Sierra and Müller's (2014) [SoilR](http://cran.r-project.org/web/packages/SoilR/index.html) package, version 1.1-22.
...
Schädel et al. (2013) consider one, two, and three pool models in their ability to model the observed data.  Sierra and Müller (2014) present a two-pool model with decomposition rates for each pool as well as transfer rates from one pool to another. 
...
Because the two pools are theoretical constructs to account for soil heterogeneity, they are not measured directly.  Instead, only the total initial carbon $C_1(0) + C_2(0)$ is observed, not how it is partitioned between the two pools.  

##### Evolved Carbon Measurement

Then, for each measurement time $t_n$, the total carbon evolved (i.e., respirated out of the system) since $t_{n-1}$ is measured, i.e.,
$\left[ C_1(t_{n-1}) + C_2(t_{n-1}) \right] - \left[ C_1(t_n) + C_2(t_n) \right]$.

The data set downloaded from the SoilR package provides, for each time $t_n$, the total carbon evolved in the form of carbon dioxide (CO<sub>2</sub>) since $t=0$,
$$
\mbox{eCO}_2(t_n) = \left[ C_1(0) + C_2(0) \right] - \left[ C_1(t_n) + C_2(t_n) \right].
$$
...
Weakly informative priors are supplied for all parameters, whose rough values are known from prior experiments.
...
$$y_t \sim \mbox{Normal}(\hat{y}_t, \sigma),$$

where $y_t$ is the measured eCO<sub>2</sub> and $\hat{y}_t$ is the estimate of eCO<sub>2</sub> derived from the solution to the system equations given initial condition $C(0)$ and model parameters $(\gamma,\kappa,\alpha,\sigma)$.
...
$$
p(\gamma,\kappa,\alpha,\sigma \, | \, y,C(0))
\ \propto \ p(y \, | \, \gamma,\kappa,\alpha,\sigma,C(0)) \ p(\gamma,\kappa,\alpha,\sigma).
$$
Here, $y_t$ is the cumulative $\mbox{eCO}_2(t)$ measurements provided in the data.
...
```{r comment=NA}
lines <- 
  readLines("/Users/carp/github/soil-metamodel/stan/soil-incubation/soil_incubation.stan",
            encoding="ASCII");
...
0.  Make sure the parameterization is consistent.  I think $\alpha_{1,2}$ and $\alpha_{2,1}$ may have been swapped from Sierra and Müller's (2014) presentation.

1. Evaluate a reparameterization of the models in terms of independent $a_{2,1} = \alpha_{2,1} \, \kappa_2$ and $a_{1,2} = \alpha_{1,2} \, \kappa_1$.  Alternatively, consider total decay $\kappa_1$ for pool 1 and then split that with a parameter in $(0,1)$ between evolution and pool 2.

2.  Move noise to log scale to respect positivity constraint.  This will make it multiplicative instead of additive;  verify this with exploratory data analysis comparing residual to value.  

3.  Implement a measurement error model for each aggregated observation.  This will presuppose an underlying true carbon concentration which is measured with error on several samples to produce the reported mean and standard deviations.  Figure out if this provides more accurate estimates or less uncertainty.

4. Evaluate a measurement error model that directly models observed measurements, 
$$
\left[ C_1(t_n) + C_2(t_n) \right] - \left[ C_1(t_{n-1}) + C_2(t_{n-1}) \right].
$$
First validate this is what's actually being measured---CO<sub>2</sub> emitted since last measurement.  I don't see how it could be given that eCO2mean measuremens can go down from point to point and I don't think they could've measured a negative concentration, even by averaging.
...
6.  Evaluate mixture model at observation level of both.

7.  Evaluate change point model going from forward to reverse.
